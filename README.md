...
- **5 боссов** (по два логова каждого, всего 10) со своими паттернами атак (bullet-hell) и анимацией ходьбы/атаки:
...
- **Пиксель-арт**, который целиком генерируется кодом при загрузке (`public/pixel.js`): никаких картинок и чужих ассетов.
...
public/pixel.js   генератор пиксель-арта: спрайты героев/монстров из текстовых сеток, процедурные
                  деревья, камни, здания, боссы и земля биомов (дизеринг, освещение, авто-контур)
public/client.js  рендер, предсказание движения со сверкой по серверу, интерполяция, интерфейс
public/fonts/     пиксельные шрифты Tiny5 и Press Start 2P (SIL Open Font License)
...
- Мир рисуется в маленький буфер (1 арт-пиксель = 3 единицы мира) и увеличивается в целое число раз
  без сглаживания, поэтому все пиксели одинаковые и чёткие; текст рисуется поверх в экранном разрешении.
- Спрайт героя — это строки вида `'..gsssssesg.'`, где каждая буква — цвет палитры (`T` — цвет команды).
  Хотите перерисовать персонажа — правьте сетку в `public/pixel.js`.
